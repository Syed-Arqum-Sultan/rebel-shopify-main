...
from .helpers import choose_output_ext, ensure_rgb, ensure_rgba, flatten_rgba_on_color, hex_to_rgb
...
def process_image(
    ...
    alpha_band = rgba.split()[3]
    has_transparent_pixels = alpha_band.getextrema()[0] < 255
    scene = profile_scene(image)
    warnings: list[str] = list(bg_warnings)

    if preset.background_mode != "transparent":
        analysis_rgb = flatten_rgba_on_color(rgba, preset.pad_color)
        metrics = analyze_photo(analysis_rgb)
        plan = build_adjustment_plan(metrics, preset.config, scene=scene)

        rgba_for_work = rgba
        shadow_baked_early = False
        if has_transparent_pixels and plan.contact_shadow_opacity > 0:
            rgba_for_work = apply_contact_shadow(rgba, plan)
            shadow_baked_early = True
        work_rgb = flatten_rgba_on_color(rgba_for_work, preset.pad_color)

        subject_mask, background_mask = build_subject_mask(image)
        normalized = apply_normalization(work_rgb, plan)
        graded = apply_grade(
            normalized,
            plan,
        ...
        final_rgba = ensure_rgba(relit) if shadow_baked_early else apply_contact_shadow(relit, plan)
    else:
        metrics = analyze_photo(rgba)
        plan = build_adjustment_plan(metrics, preset.config, scene=scene)
        subject_mask, background_mask = build_subject_mask(image)
...
def fit_to_square(
    ...
    if background_mode == "transparent":
        ...
        rgb = (0, 0, 0) if force_black_output else hex_to_rgb(pad_color)
